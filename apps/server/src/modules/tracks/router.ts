...
const zVersionType = z.enum(versionTypes);
...
export const tracksRouter = {
    ...
    .handler(async ({ input }) => {
      ...
      if (input.versionType) {
        const idsRes = await db
          .select({ id: trackVersions.trackId })
          .from(trackVersions)
          .where(eq(trackVersions.type, input.versionType))
          .groupBy(trackVersions.trackId);
      ...
      const base = where ? db.select().from(tracks).where(where) : db.select().from(tracks);
      const rows = await base.orderBy(orderBy).limit(input.limit).offset(input.offset);

      const totalRow = await (where
        ? db.select({ c: count() }).from(tracks).where(where)
        : db.select({ c: count() }).from(tracks));
      ...
      const albumsRows = ids.length
        ? await db.select().from(albums).where(inArray(albums.id, rows.map((r) => r.albumId)))
        : [];
  ...
  addVersion: adminProcedure
    ...
    .handler(async ({ input }) => {
      const maxRows = await db
        ...
        .limit(1);
      const nextIndex = ((maxRows[0]?.orderIndex ?? -1) + 1);
  ...
  updateVersion: adminProcedure
    ...
    .handler(async ({ input }) => {
      await db
        ...
        .set({ type: input.type, title: input.title, artists: input.artists, fileUrl: input.fileUrl })
        .where(eq(trackVersions.id, input.id));
      const updated = await db.select().from(trackVersions).where(eq(trackVersions.id, input.id)).limit(1);
      if (updated.length === 0) throw new Error("Not found");
      return updated[0];
    }),

  removeVersion: adminProcedure
    .input(z.object({ id: zId }))
    .handler(async ({ input }) => {
  ...
  reorderVersions: adminProcedure
    .input(
      z.object({
        trackId: zId,
